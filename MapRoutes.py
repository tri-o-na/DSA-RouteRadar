...
class FirstScreen(Screen):
    ...
    def bind_text_inputs(self, dt):
        ...
        if hasattr(self.ids, 'text_input1') and hasattr(self.ids, 'text_input2'):
            self.ids.text_input1.bind(text=self.on_text_change)
            self.ids.text_input2.bind(text=self.on_text_change)
    ...
    def on_text_change(self, instance, value):
        ...
        # If input is empty or has 3 characters already, hide suggestions
        if not value or len(value) >= 3:
            if self.suggestion_dropdown:
                self.suggestion_dropdown.dismiss()
                self.suggestion_dropdown = None  # Clear the reference
            return
        
        query = value.strip().upper()
        
        matching_codes = [code for code in self.airport_codes if code.startswith(query)]
        ...
        # Dismiss existing dropdown before creating a new one
        if self.suggestion_dropdown:
            self.suggestion_dropdown.dismiss()
    ...
    def show_airport_list(self):
        ...
        # self.ids.airport_list_label.text = airport_list_text  # Update label text
        if not self.dialog:
            ...
            self.dialog.text = airport_list_text  # Update text dynamically
        self.dialog.open()
    def hide_airport_list(self):
        """Clear the label when user moves away."""
        self.ids.airport_list_label.text = ""  # Clear text
    
    def on_touch_move(self, touch):
        """Detects when the mouse moves away and hides the airport list."""
        if not self.collide_point(*touch.pos):  # If the touch moves outside the widget
...
class SecondScreen(Screen):
    ...
    def display_routes(self, filtered_routes):
        ...
        for route in unique_routes.values():
            ...
            distance_label = MDLabel(
                text=f"Distance: {distance:.0f}km",
                font_style="Body2",
                theme_text_color="Secondary"
            )
    ...
    def reset_filters(self):
        ...
        if hasattr(self.manager, "routes") and self.manager.routes:
            # Display all routes after resetting
            self.display_routes(self.manager.routes)
